...
Если сервер заведомо без GPU и веса `torch`/`torchvision`/`torchhull`
(~4-5 ГБ) не нужны — можно убрать эти строки и `triton`/`nvidia-*` из
`requirements.txt` перед установкой; приложение продолжит работать в режиме
«без автоматической классификации», описанном выше.
