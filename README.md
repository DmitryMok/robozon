...
[Презентация решения (Трек 3, LetMeDoItSolo)](./Трек_3_Презентация_LetMeDoItSolo.pdf)
...
**Симуляция** — Webots (headless-режим, демо доступно через браузер): трение,
геометрия камер/зеркала, коллизии по упрощённым
...
### Часть тестовых STL не входит в репозиторий

11 из 16 тестовых объектов — STL, полученные при разработке от третьей
стороны (постановщика задачи), и намеренно **не публикуются** в этом
репозитории. В сборке остаются только 5 синтетических пограничных
объектов (`asym_barrel`, `asym_cone`, `asym_cyl`, `cyl_skewed`, `ell_cyl`) —
их достаточно для проверки кода и алгоритма, но не для полноценного прогона
демо со всем набором.

Чтобы получить полный набор из 16 объектов, нужно добавить эти 11 файлов
самостоятельно по трём путям (везде одинаковый набор из 11 файлов):

- `robozon/assets/meshes/`
- `robozon/assets/stl/`
- `roboson_tools/assets/stl/` (те же объекты, но с русскими именами файлов)

| Английское имя (`robozon/assets/`) | Русское имя (`roboson_tools/assets/stl/`) |
|---|---|
| `bag.stl` | `Мешок.stl` |
| `bottle.stl` | `Бутылка.stl` |
| `box_large.stl` | `Короб 400х400х300.stl` |
| `box_small.stl` | `Короб 300х200х200.stl` |
| `cylinder.stl` | `Цилиндр.stl` |
| `detergent.stl` | `Моющее средство.STL` |
| `helmet.stl` | `Шлем.stl` |
| `lunchbox.stl` | `ЛанчБокс.stl` |
| `pen.stl` | `Ручка.stl` |
| `plate.stl` | `Тарелка.stl` |
| `pouf.stl` | `Пуфик.stl` |

Без этих файлов исполнительный механизм спавнит только 5 синтетических
объектов, а сценарии проверки CV-отчёта ниже, ссылающиеся на `plate.stl`,
`lunchbox.stl`, `box_large.stl`, `box_small.stl`, недоступны до тех пор, пока
файлы не будут добавлены.
...
- Классификация должна завершиться заметно раньше, чем объект доедет до
  ...
  работа потока подтверждена на подаче объекта раз в 0.7 с.
...
(пункты 2-4 используют STL, не входящие в репозиторий — см. «Часть тестовых
STL не входит в репозиторий» выше, недоступны без них)
1. Загрузить явно круглый объект (например `robozon/assets/stl/ell_cyl.stl`,
   входит в репозиторий, или `plate.stl`) — ожидаемая категория D, k ≥ 0.8.
...
4. Загрузить объект-ловушку, похожий на куб/короб по диагонали (`box_small.stl`
   или входящий в репозиторий `cyl_skewed.stl`) — показательный тест на
   «обманчивую» круглость: с GPU sustained-check должен корректно понизить
   вердикт до `uncertain`/`not_round`, без GPU автоматическая классификация не
...
7. Автоматическая подача через веб-панель — сама панель может тормозить
   ...
   (устойчивость на темпе 0.7 с уже проверена отдельно). Через панель
   достаточно убедиться, что товар физически
   маршрутизируется в свою зону и не застревает — не пытаться воспроизвести
   через неё высокий темп подачи.

## Известные особенности

- **Открытый вопрос — смягчение падения товара в ролл-кейдж.** Высота
  падения с конвейера на дно кейджа — около 0.8 м; конкретное решение пока
  не выбрано — правильный вариант зависит от конкретной модели используемых
  ролл-кейджей, под неё и стоит проектировать смягчение, а не выбирать
  вслепую. Рассматриваемые
  направления (без окончательного выбора): наклон самого кейджа, откидной
  борт, складной борт (сначала работает как пологий спуск, после заполнения
  складывается и становится обычной вертикальной стенкой).
